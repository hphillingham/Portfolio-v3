...
const global_retrievedProjectDataOverwrites = {
	...
	conquerors_of_catan: {
		title: "Extending the MiniMax algorithm to play The Settlers of Catan.",
	...
	hikers_challenge: {
		...
		image:
			"https://media.githubusercontent.com/media/hphillingham/Hikers-Challenge/refs/heads/master/assets/icon-circle-border.png",
	},
	portfolio_v3: {
		title:
...
function increaseActivityDataLength() {
	...
	if (global_currentActivityLength >= global_activityLengthLimit) {
		document
			.getElementById("ID_loadMoreActivityButton")
...
async function getPublicGitHubRepos() {
	...
	global_extractedRepoData = data.repos
		...
		.map((repo) => {
			...
			const repoOverwriteData =
				global_retrievedProjectDataOverwrites[overwriteKey];
			return {
				name: repo.name.replaceAll("-", " "),
				title: repoOverwriteData?.title || repo.title?.split("[")[0] || "",
				image:
		...
		.sort((a, b) => {
			const indexA = global_repoDisplayFilterList.indexOf(
				a.name.replaceAll(" ", "-"),
			);
			const indexB = global_repoDisplayFilterList.indexOf(
				b.name.replaceAll(" ", "-"),
			);
...
function getDisplayDate(inputDateString, relativeDate = true) {
	...
	if (relativeDate) {
		const daysAgo = Math.floor(
			(Date.parse(currentDateTimeObject) - Date.parse(inputDateString)) /
				86400000,
		);
		...
		if (daysAgo >= 31 && daysAgo <= 365)
			return `${Math.floor(daysAgo / 30.5)} month${Math.floor(daysAgo / 30.5) > 1 ? "s" : ""} ago`;
		return "Over a year ago";
	} else {
		const currentMonth = String(currentDateTimeObject.getMonth() + 1).padStart(
			2,
			"0",
		);
		const currentDate = String(currentDateTimeObject.getDate()).padStart(
			2,
			"0",
		);
...
async function getCommitsForRepos() {
	...
	for (const repo of global_repoActivityFilterList) {
		...
		try {
			let existingData = await fetchFromCache(
				`COMMIT_DATA_${repoTarget}`,
				global_forceUpdateAll,
			);

			if (!existingData) {
				const response_branches = await fetch(
					`/data/repo-branches?repo=${repoTarget}`,
					{ method: "GET" },
				);
...
async function getGitHubActivity() {
	...
	global_extractedActivityData = data.repos
		.filter(
			(repo) =>
				repo.type !== "PushEvent" &&
				repo.type !== "BranchDeleteEvent" &&
				repo.type !== "DeleteEvent",
		)
		.map((repo) => {
			...
			const base_title_text = (() => {
				switch (repo.type) {
					...
					case "WatchEvent":
						return `Watched `;
					case "CreateEvent":
						return `Created Branch '${repo.payload.ref}' in `;
					case "PullRequestEvent":
						return `Created Pull Request #${repo.payload.number} in `;
					case "ForkEvent":
						return "Forked";
					default:
			...
			const icon = (() => {
				switch (repo.type) {
					...
					case "CreateEvent":
						return CodeBranchOutline;
					case "WatchEvent":
						return EyeOutline;
					case "PullRequestEvent":
						return CodeBranchOutline;
					case "ForkEvent":
						return CodeForkOutline;
					default:
						return CodeBranchOutline;
				}
			})();
	...
	global_extractedActivityData = global_extractedActivityData.slice(
		0,
		global_activityLengthLimit,
	);
